/**
 * Cyprus Access Control (CAC) - QR Code Routes
 * Handles QR code generation, validation, and activation workflows
 */

import { Router, Request, Response } from 'express';
import { QRCodeService } from '../services/qrCode';
import geofencingService from '../services/geofencing';
import { LocationValidationRequest } from '../types/extended';
...
const activationRateLimit = rateLimit({
  ...
  max: 3, // 3 activation attempts per hour
 ...
 */
router.post('/generate', qrRateLimit, async (req: Request, res: Response) => {
  try {
    const {
      invitationNumber,
      expirationHours = 72,
    ...
    // Validate invitation number format (CYH-001, CYR-002, CYC-003)
...
/**
 * QR code landing page with activation interface
 * GET /qr/:invitationNumber
 */
router.get('/:invitationNumber', async (req: Request, res: Response) => {
  try {
    ...
    // Validate QR token
    const validation = await qrService.validateQRCode(token as string);
 ...
 * POST /api/qr/sms/send
 */
router.post('/sms/send', qrRateLimit, async (req: Request, res: Response) => {
  try {
...
/**
 * Verify SMS code
 * POST /api/qr/sms/verify
 */
router.post('/sms/verify', qrRateLimit, async (req: Request, res: Response) => {
  try {
...
/**
 * Activate invitation with full verification
...
router.post('/activate', activationRateLimit, async (req: Request, res: Response) => {
  try {
    ...
    if (qrValidation.metadata?.locationRequired && locationData) {
      const locationRequest: LocationValidationRequest = {
        invitationNumber,
        coordinates: locationData,
        timestamp: activationTime || Date.now(),
        deviceInfo: {
          userAgent: req.headers['user-agent'] || '',
          timezone: 'UTC', // Would be passed from client
